...
TL;DR: if an object inherits behavior from `ActiveModel::Model` or defines `delegate :persisted?, :to_model, to:` an instance that inherits behavior from `ActiveModel::Model`, the `form_with` method can effectively figure out the resource name, action, and, if needed, the resource ID.
